...
//        $setting = Mailsetting::find()->select(['setting_value'])->all();
        ...
        foreach ($datas as $mails) {
            ...
            $result = Yii::$app->mailer->compose(
                            [
                        'html' => '/templates/_email-html',
                        'text' => '/templates/_email-text',
                            ], $params)
                    ->setTo($mails['to'])
                    ->setFrom(Yii::$app->params['robotEmail'])
                    ->setReplyTo(Yii::$app->params['robotEmail'])
                    ->setSubject($mails['subject'])
                    ->setTextBody($mails['message_body'])
                    ->send();
    ...
    public function actionDeleteMultiple() {
        $pk = Yii::$app->request->post('ids'); // Array or selected records primary keys
